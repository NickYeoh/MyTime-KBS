    ...
    public class AttendanceDBService
    {
        ...
        public List<AttendanceModel> GetMonthlyAttendance(string NRIC, string usrID, string userName, string departmentID, string departmentName, DateTime startOn, DateTime endOn, int accessRoleID, string approverName)
        {
            ...
            try
            {
                ...
                // Check AttendanceTrans_xxxx is exist - generated by auto closing
                attendanceTransTableName = "AttendanceTrans_" + startOn.ToString("yyyyMM");

                string sql = "SELECT * FROM INFORMATION_SCHEMA.TABLES";
                sql += " " + "WHERE TABLE_SCHEMA = 'dbo'";
                sql += " " + "AND TABLE_NAME = '" + attendanceTransTableName + "'";
                ...
                if (dr.HasRows)
                {
                    isAttendanceTransTableFound = true;
                ...
                if (isAttendanceTransTableFound == true)
                {
                    ...
                    //      ,T.TimeInEnd
                    //      ,T.ShiftDuration
                    //      ,T.AttendanceStatusID
                    //      ,T.FirstIn
                    //      ,T.LastOut
                    //      ,T.Lateness
                    //      ,T.WorkTime
                    //      ,T.OverTime
                    //      ,T.OverTimeExtraStart
                    //      ,T.OverTimeExtraEnd
                    //      ,T.OverTimeExtra
                    //	  ,AR.Proof, AR.ReasonID, R.ReasonName, AR.Remark, AR.IsApproved, AR.IsRejected, AR.IsRequestedToAmend, AR.ApproverComment
                    //  FROM AttendanceTrans_202203 T
                    //  LEFT JOIN AttendanceReason_202203 AR
                    //  ON AR.AttendanceDate = T.AttendanceDate AND AR.NRIC = T.NRIC
                    //  LEFT JOIN Reason R
                    ...
                    //  WHERE T.NRIC = '790202105476'
                    //  ORDER BY T.NRIC, T.AttendanceDate
